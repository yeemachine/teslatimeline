...
$(document).ready(function(){
    ...
    $('.charging-legal .kwh-price').text(formatMoney(PARAMS["cost"]));
    $('.charging-legal .mpg-price').text(formatMoney(PARAMS["gas"]));
    ...
    function calculateWall(){
        ...
        calculatedValues.cost = formatMoney(cost);
        calculatedValues.savings = formatMoney(gasSavings, '$', 2);
    }
...
;
/**
 * In order to prevent form duplication on the same page, in cases where both a mobile and
 * desktop version of the same form is required, we should only load the desktop form
 * and move it in the DOM if the window is resize to mobile.
 */
$(function() {
    // On the homepage, remove the test drive form (used in AB test).
    // TODO: Remove the whenever we end the Optimizely AB tests for the mobile form
    // and remove test drive form from mobile.
    $('.page-homepage .test-drive-container').html("");

    // Get jQuery selectors
    var $stickyNavForm = $('.sticky-nav #tesla-insider-form');
    var $mobileForm = $('.insider-container #tesla-insider-form');

    // Moves the get updates form around the Model X and 3 page if there's multiple
    // copies of the form in the DOM.
    if (($('.page-model3').length > 0 || $('.page-modelx').length > 0) && ($stickyNavForm.length > 0 && $mobileForm.length > 0)) {
    ...
    // Push GTM analytics for mobile
    $mobileForm.submit(function() {
        window.dataLayer.push({
            'event': 'form-interaction',
            'interaction': 'confirm-thank-you',
        ...
        attach: function () {
            ...
            if ($form.length) {
                ...
                $('#tesla-insider-modal').on('show.bs.modal', function (event) {
                    debug.log('on show | GTM: form-interaction, view-open, Newsletter Signup (Tesla Insider), User Requested');
                    window.dataLayer.push({
                        'event': 'form-interaction',
        ...
        $('#edit-location').change(function() {
            ...
            for (var i in locations) {
                ...
                if (loc.id == location_id) {
                    var slug = Drupal.settings.tesla.localePrefix + '/';
                    $('#map-address').html('<strong>' + Drupal.t('Tesla !location_name', {'!location_name': $('#edit-location option[value="' + location_id + '"]').text()}) + '</strong><br>' + loc.map_address + '<br><a class="driving-directions" href="' + slug + loc.service_id + '" target="_blank" rel="noopener noreferrer">' + Drupal.t('Hours and directions') + '</a>');
                    for (var j in loc.dates) {
                        var formattedDate = loc.dates[j].formatted;
                        $('#edit-preferred-date').append('<option value="' + j + '">' + formattedDate + '</option>');
